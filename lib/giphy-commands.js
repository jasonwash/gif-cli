...
const GIPHYCommandClient = function (apiKey) {
  ...
  return {
    search: (query, isStickers) => {
      ...
      return client.search(gifsOrStickers(isStickers), {"q": query, limit: 10})
        .then((response) => {
    ...
    trending: (isStickers) => {
      return client.trending(gifsOrStickers(isStickers), {})
        .then((response) => {
          // console.log(`Trending response.data:  ${JSON.stringify(response.data, null, 2)}`);
          const dataIndex = getRandomInt(response.data.length);
          return rendition(response.data[dataIndex]).url;
        })
        .then((gifUrl) => {
